...
- extracted_attributes.csv: One row per attribute value across all levels
...
class DatabaseReader:
    ...
    def get_cancer_files(self, limit: int = None, chunk_size: int = 1000) -> Dict[str, Any]:
        """
        ...
        while True:
            ...
            if not rows:
                logger.info(f"All files retrieved from database")
                break
    ...
    def get_concept_files(self, limit: int = None, chunk_size: int = 1000) -> Dict[str, Any]:
        """
...
class CancerDataExtractor:
    ...
    def _extract_attribute(self, attr: Dict[str, Any], patient_id: str, cancer_idx: int,
        ...
        for val_idx, value_obj in enumerate(values):
            record = {
                'patient_id': patient_id,
                'cancer_index': cancer_idx,
    ...
    def save_to_csv(self) -> None:
        ...
        if self.tumors_data or self.attributes_data or self.cancers_data:
            ...
            self._write_csv_files()
            logger.info(f"✓ Final cancer data written")
...
class ConceptExtractor:
    ...
    def save_to_csv(self) -> None:
        ...
        if self.concepts_data:
            ...
            self._write_csv_files()
            logger.info(f"✓ Final concept data written")
...
def process_cancer_data(db_path: str, output_dir: Path, cancer_count: queue.Queue) -> None:
    """
    ...
    try:
        ...
        for cancer_files in db_reader.get_cancer_files(limit=None):
            ...
            logger.info(f"[CANCER THREAD] Cancer chunk extraction complete")

        if cancer_extractor:
            logger.info(f"[CANCER THREAD] Saving final cancer data...")
...
def process_concept_data(db_path: str, output_dir: Path, concept_count: queue.Queue) -> None:
    """
    ...
    try:
        ...
        for concept_files in db_reader.get_concept_files(limit=None):
            ...
            logger.info(f"[CONCEPT THREAD] Concept chunk extraction complete")

        if concept_extractor:
            logger.info(f"[CONCEPT THREAD] Saving final concept data...")
...
def main():
    ...
    logger.info(f"Processing mode: 2 parallel threads (cancer + concepts)")
    logger.info("="*80)

    try:
        # Queues to track completion
        cancer_count = queue.Queue()
        concept_count = queue.Queue()
        ...
        logger.info(f"  extracted_cancer_data/")
        logger.info(f"    extracted_tumors/")
        logger.info(f"      extracted_tumors_1.csv, extracted_tumors_2.csv, etc.")
        logger.info(f"    extracted_attributes/")
        logger.info(f"      extracted_attributes_1.csv, extracted_attributes_2.csv, etc.")
        logger.info(f"    extracted_cancers/")
        logger.info(f"      extracted_cancers_1.csv, extracted_cancers_2.csv, etc.")
        logger.info(f"    extracted_concepts/")
        logger.info(f"      extracted_concepts_1.csv, extracted_concepts_2.csv, etc.")
        logger.info(f"  (Each file contains data for up to 1000 patients)")
