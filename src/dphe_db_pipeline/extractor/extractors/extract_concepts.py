#!/usr/bin/env python3
"""
Script to extract JSON content from the 'files' table where filenames end with '_Concepts'.
"""
...
def extract_concepts_content(db_path, output_dir="extracted_concepts"):
    """
    Extract all content from files table where filename ends with '_Concepts'.
    ...
    # Extract content from files ending with _Concepts
    print("\n" + "="*60)
    print("Extracting content from files ending with '_Concepts'...")
    print("="*60)

    # Query for files ending with _Concepts
    query = "SELECT * FROM files WHERE filename LIKE '%_Concepts.json'"
    cursor.execute(query)

    rows = cursor.fetchall()

    if not rows:
        print("No files found ending with '_Concepts'")
        conn.close()
        return

    print(f"Found {len(rows)} file(s) ending with '_Concepts'\n")
    ...
    if encoding_idx is not None:
        print("✓ Found 'encoding' column - will use it to decode content")
    ...
    for idx, row in enumerate(rows, 1):
        ...
        try:
            if content:
                ...
                if encoding and encoding.lower() == 'zstd':
                    if not ZSTD_AVAILABLE:
                        print("   ✗ zstandard library not installed. Run: pip install zstandard")
                        continue
                    ...
                    if isinstance(content, bytes):
                        ...
                        print("   ⚠ Content is not bytes, but encoding is zstd")
                        content_str = content
                else:
            ...
            for fallback_encoding in ['latin-1', 'iso-8859-1', 'cp1252', 'utf-16']:
                ...
                print("   ✗ Could not decode content with any common encoding")
    ...
    # Save all concepts in a single file
    if all_concepts:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        combined_file = os.path.join(output_dir, f"all_concepts_{timestamp}.json")
        with open(combined_file, 'w', encoding='utf-8') as f:
            json.dump(all_concepts, f, indent=2, ensure_ascii=False)

        print("\n" + "="*60)
        print(f"✓ Saved all concepts to: {combined_file}")
        print(f"✓ Total files processed: {len(all_concepts)}")
        print("="*60)
...
def main():
    ...
    concepts = extract_concepts_content(db_path)

    if concepts:
        print(f"\nSuccessfully extracted {len(concepts)} concept file(s)!")
    else:
