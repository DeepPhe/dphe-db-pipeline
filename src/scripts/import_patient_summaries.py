...
def resolve_paths(
    ...
    resolved_data_dir = data_dir or (base_dir / "extracted_cancer_data")
    resolved_db_path = db_path or (base_dir / "deepphe" / "deepphe_sqlite_compressed")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Import extracted_cancer_data/patient_summaries.jsonl into SQLite."
    )
    parser.add_argument(
        "--db-path",
        type=Path,
        default=None,
        help="Path to the target SQLite DB (default: deepphe/deepphe_sqlite_compressed).",
    )
    parser.add_argument(
        "--jsonl-path",
        type=Path,
