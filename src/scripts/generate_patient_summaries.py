...
Writes:
  extracted_cancer_data/patient_summaries.jsonl
...
try:
    from patient_summaries.bitmap_index import (
        preload_attributes,
        preload_cancers,
        preload_concepts,
        preload_tumors,
    )
    from patient_summaries.demographics import load_demographics
    from patient_summaries.models import PatientSummary, _hit_to_dict
    from patient_summaries.postprocess import dedup_and_merge
except ImportError:
...
def main() -> int:
    ...
    default_out = base_dir / "extracted_cancer_data" / "patient_summaries.jsonl"
